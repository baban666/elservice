...
class VP_Metabox extends WPAlchemy_MetaBox
{
	...
	function __construct($arr)
	{
		...
		if( $this->is_dev_mode )
		{
			$this->title = __('[Development Mode] ', 'elservice') . $this->title;
		}
	...
	function _render_repeating_group($group)
	{
		...
		foreach ($group['groups'] as $g)
		{
			...
			$html .= '<div class="vp-wpa-group-heading"><a href="#" class="vp-wpa-group-title">' . $icon . $group['title'] . '</a><a href="#" class="dodelete vp-wpa-group-remove" title="'. __('Remove', 'elservice') .'"><i class="far fa-times"></i> '. __('Remove', 'elservice') .'</a></div>';
			$html .= '<div class="vp-controls' . ((!$is_first) ? ' vp-hide' : '') . '">';
		...
		$html .= '<a href="#" class="button button-primary docopy-' . $name . '">'. __('Add More', 'elservice') . ' ' . $group['title'] . '</a>';
		$html .= '</div>';
